...
std::unique_ptr<SoundStreamEncoder> SoundStreamEncoder::Create(
  ...
  auto model = TfLiteModelWrapper::Create(
      model_path / "soundstream_encoder.tflite", /*use_xnn=*/true, /*int8_quantized=*/true);
  if (model == nullptr) {
    std::cerr << "Unable to create SoundStream encoder TFLite model wrapper.";
    return nullptr;
  }
...
std::unique_ptr<SoundStreamEncoder> SoundStreamEncoder::Create(
    const char* encoder_buffer, uint64_t encoder_buffer_size) {
  auto model =
      TfLiteModelWrapper::Create(encoder_buffer, encoder_buffer_size, /*use_xnn=*/true, /*int8_quantized=*/true);
  if (model == nullptr) {
    std::cerr << "Unable to create SoundStream encoder TFLite model wrapper.";
    return nullptr;
  }
...
std::optional<std::vector<float>> SoundStreamEncoder::Extract(
  ...
  if (!model_->Invoke()) {
    std::cerr << "Unable to invoke SoundStream encoder TFLite model wrapper.";
    return std::nullopt;
  }
