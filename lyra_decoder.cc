...
inline int GetConcealmentDurationSamples() {
  ...
  if (kConcealmentDurationSamples %
          GetNumSamplesPerHop(kInternalSampleRateHz) !=
      0) {
    std::cerr << "Concealment duration must be a multiple of hop size."
              << std::endl;
    exit(1);
  }
...
inline int GetFadeDurationSamples() {
  ...
  if (kFadeDurationSamples % GetNumSamplesPerHop(kInternalSampleRateHz) != 0) {
    std::cerr << "Fade duration must be a multiple of hop size." << std::endl;
    exit(1);
  }
...
std::unique_ptr<LyraDecoder> LyraDecoder::Create(
  ...
  if (resampler == nullptr) {
    std::cerr << "Could not create Buffered Resampler.";
    return nullptr;
  }
  ...
  if (model == nullptr) {
    std::cerr << "New model could not be instantiated.";
    return nullptr;
  }
  ...
  if (comfort_noise_generator == nullptr) {
    std::cerr << "Could not create Comfort Noise Generator.";
    return nullptr;
  }
...
std::unique_ptr<LyraDecoder> LyraDecoder::Create(
  ...
  absl::Status are_params_supported =
      AreParamsSupported(sample_rate_hz, num_channels);
  if (!are_params_supported.ok()) {
    std::cerr << are_params_supported;
    return nullptr;
  }
  ...
  // All internal components operate at |kInternalSampleRateHz|.
  auto model =
      CreateGenerativeModel(kNumSamplesPerHop, kNumFeatures, model_buffer);
  if (model == nullptr) {
    std::cerr << "New model could not be instantiated.";
    return nullptr;
  }
...
bool LyraDecoder::SetEncodedPacket(absl::Span<const uint8_t> encoded) {
  ...
  if (num_quantized_bits < 0) {
    std::cerr << "The packet size (" << encoded.size()
  ...
  if (!unpacked.has_value()) {
    std::cerr << "Could not read Lyra packet for decoding.";
    return false;
  }
  ...
  if (!features.has_value()) {
    std::cerr << "Could not decode to lossy features.";
    return false;
  }
  if (!generative_model_->AddFeatures(features.value())) {
    std::cerr << "Could not add received features to generative model.";
    return false;
  }
...
std::optional<std::vector<int16_t>> LyraDecoder::DecodeSamples(
  ...
  if (!external_samples.has_value()) {
    std::cerr << "Could not decode samples.";
    return std::nullopt;
  }
...
std::optional<std::vector<int16_t>> LyraDecoder::DecodeSamplesInternal(
  ...
  while (result.size() < internal_num_samples_to_generate) {
    ...
    if (!comfort_noise.has_value()) {
      std::cerr << "Could not generate comfort noise.";
      return std::nullopt;
    }
    ...
    if (!MaybeOverlapAndInsert(fade_direction_, fade_progress_, audio.value(),
      ...
      std::cerr << "Could not overlap comfort noise.";
      return std::nullopt;
    }
    ...
    if (is_packet_received) {
      if (!noise_estimator_->ReceiveSamples(audio.value())) {
        std::cerr << "Could not update noise estimator on decoder output.";
        return std::nullopt;
      }
    }
  }
  if (result.size() != internal_num_samples_to_generate) {
    std::cerr << "Result size does not match requested size.";
    exit(1);
  }
  return result;
}
...
std::optional<std::vector<int16_t>> LyraDecoder::RunGenerativeModel(
  ...
  if (num_samples > 0 && generative_model_->num_samples_available() == 0) {
    if (!generative_model_->AddFeatures(feature_estimator_->Estimate())) {
      std::cerr << "Could not add estimated features to generative model.";
      return std::nullopt;
    }
  }
...
bool LyraDecoder::MaybeOverlapAndInsert(
  ...
  if (generative_model_hop.size() != comfort_noise_hop.size()) {
    std::cerr << "Overlapped hop could not be computed because hop sizes "
