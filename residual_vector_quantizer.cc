...
std::unique_ptr<ResidualVectorQuantizer> ResidualVectorQuantizer::CreateQuantizerFromModel(std::unique_ptr<TfLiteModelWrapper> quantizer_model) {
  if (quantizer_model == nullptr) {
    std::cerr << "Unable to create the quantizer TfLite model wrapper.";
    return nullptr;
  }
  tflite::SignatureRunner* encode_runner =
      quantizer_model->GetSignatureRunner("encode");
  if (encode_runner == nullptr) {
    std::cerr << "The quantizer TFLite model has no encode signature";
    return nullptr;
  }
  if (encode_runner->AllocateTensors() != kTfLiteOk) {
    std::cerr << "Could not allocate encode runner TFLite tensors.";
    return nullptr;
  }
  tflite::SignatureRunner* decode_runner =
      quantizer_model->GetSignatureRunner("decode");
  if (decode_runner == nullptr) {
    std::cerr << "The quantizer TFLite interpreter has no decode signature";
    return nullptr;
  }
  if (decode_runner->AllocateTensors() != kTfLiteOk) {
    std::cerr << "Could not allocate decode runner TFLite tensors.";
    return nullptr;
  }
...
std::optional<std::string> ResidualVectorQuantizer::Quantize(
 ...
 if (num_bits > kMaxNumQuantizedBits) {
   std::cerr << "The number of bits cannot exceed maximum ("
              << kMaxNumQuantizedBits << ").";
   return std::nullopt;
 }
 if (num_bits % bits_per_quantizer_ != 0) {
   std::cerr << "The number of bits (" << num_bits
 ...
 if (encode_runner_->Invoke() != kTfLiteOk) {
   std::cerr << "Unable to invoke the quantize runner.";
   return std::nullopt;
 }
...
ResidualVectorQuantizer::DecodeToLossyFeatures(
 ...
 if (decode_runner_->AllocateTensors() != kTfLiteOk) {
   std::cerr << "Unable to allocate tensors.";
   return std::nullopt;
 }
 ...
 if (decode_runner_->Invoke() != kTfLiteOk) {
   std::cerr << "Unable to invoke the decode runner.";
   return std::nullopt;
 }
