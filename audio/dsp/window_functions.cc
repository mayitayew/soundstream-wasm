...
WindowFunction::WindowFunction(double radius): radius_(radius) {
  if (radius <= 0.0) {
    std::cerr << "Radius must be a positive double." << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
QuarticWindow::QuarticWindow(double radius, double c_2, double c_4)
    : WindowFunction(radius),
      c_2_(c_2),
      c_4_(c_4) {
//  CHECK_GE(c_2, -2.0);  // Falling faster than this can't be compensated by c_4.
//  CHECK_LE(c_2, 0.0);  // Keeps peak at center.
//  CHECK_GE(c_4, -(1 + c_2));  // Keeps window nonnegative at ends.
//  CHECK_LE(c_4, -c_2 / 2);  // Prevent slope reversal at ends to keep unimodal.
}
