...
//  DCHECK(variance);
  ...
  if (num_samples <= 0) {
    std::cerr << "FATAL: Num samples must be greater than 0. " << std::endl;
    exit(EXIT_FAILURE);
  }
...
//  CHECK(in2);
  ...
  if (in1 == nullptr || in2 == nullptr) {
    std::cerr << "FATAL: nullptr not allowed. " << std::endl;
    exit(EXIT_FAILURE);
  }
...
double RootMeanSquareError(const ContainerType1& in1,
  ...
  //CHECK_EQ(n_elements, in2.size());

  if (n_elements != in2.size()) {
    std::cerr << "FATAL: n_elements must be equal to in2 size. " << std::endl;
    exit(EXIT_FAILURE);
  }
...
void FindMaxIndexAndValue(const ContainerType& container,
  ...
  //DCHECK(max_index && max_value);
  //CHECK(!container.empty());

  if (container.empty()) {
    std::cerr << "FATAL: container cannot be empty." << std::endl;
    exit(EXIT_FAILURE);
  }
...
void ElementwiseMax(const ContainerType1& other, ContainerType2* target) {
  //DCHECK(target);
  //CHECK_EQ(other.size(), target->size());

  if (other.size() != target->size()) {
    std::cerr << "FATAL: other size is not equal to target size." << std::endl;
    exit(EXIT_FAILURE);
  }
...
//  CHECK_GE(coefficient, 0);
  ...
  if (signal == nullptr || scale < 0 || coefficient < 0) {
    std::cerr << "FATAL: signal, scale or coefficient invalid. " << std::endl;
    exit(EXIT_FAILURE);
  }
...
float QuadraticInterpolateMax(const ContainerType& signal, int x0,
                              float *max_location) {
//  CHECK_GE(x0, 0);
//  CHECK_LT(x0, signal.size());
//  CHECK(max_location);

  if (x0 < 0 || x0 >= signal.size() || max_location == nullptr) {
    std::cerr << "FATAL: invalid args. " << std::endl;
    exit(EXIT_FAILURE);
  }
...
double RealDotProduct(
  ...
  if (n_elements != in2.size()) {
    std::cerr << "FATAL: n_elements not equal to in2 size." << std::endl;
    exit(EXIT_FAILURE);
  }
...
void CombineRealAndImaginaryPartsIntoComplexVector(
  ...
  //CHECK_EQ(size, imaginary_part.size());

  if (size != imaginary_part.size()) {
    std::cerr << "FATAL: size not equal to imaginary par size." << std::endl;
    exit(EXIT_FAILURE);
  }
...
//  CHECK_GT(period, 0.0);
  ...
  if (values.empty() || period <= 0.0) {
    std::cerr << "FATAL: values empty or non-positive period. " << std::endl;
    exit(EXIT_FAILURE);
  }
...
//  CHECK_GT(period, 0.0);
  ...
  if (values.empty() || values.size() != weights.size() || period <= 0.0) {
    std::cerr << "FATAL: invalid args " << std::endl;
    exit(EXIT_FAILURE);
  }
...
class FIRFilter {
 public:
  ...
  explicit FIRFilter(const std::vector<CoefficientType>& impulse_response)
    ...
    //CHECK(!impulse_response_.empty());

    if (impulse_response_.empty()) {
      std::cerr << "FATAL: empty impulse response. " << std::endl;
      exit(EXIT_FAILURE);
    }
...
std::vector<typename ContainerType::value_type> DesignFirLowpassFilter(
  ...
  typedef typename ContainerType::value_type ValueType;
//  CHECK_GE(high_band_edge, 0.0);
//  CHECK_LE(high_band_edge, sample_rate / 2);
//  CHECK_EQ(window.size() % 2, 1);  // Window size must be odd.

  if (high_band_edge < 0.0 || high_band_edge > sample_rate / 2 || window.size() % 2 != 1) {
    std::cerr << "FATAL: invalid args" << std::endl;
    exit(EXIT_FAILURE);
  }
...
std::vector<typename ContainerType::value_type> DesignFirBandpassFilter(
  ...
  //CHECK_LE(low_band_edge, high_band_edge);

  if (low_band_edge > high_band_edge) {
    std::cerr << "FATAL: low band edge must be less than or equal to high band edge. " << std::endl;
    exit(EXIT_FAILURE);
  }
