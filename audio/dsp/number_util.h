...
//    DCHECK(0 <= i && i < size_) << "Out of bounds index " << i
//        << " in ArithmeticSequence of size " << size_ << '.';
  ...
  void CopyTo(ContainerType* output) const {
    ...
    if (size_ > kMaxSafeSize) {
      std::cerr << "FATAL: size must not be greater than max safe size. (" << kMaxSafeSize << ")" << std::endl;
      exit(EXIT_FAILURE);
    }
//    CHECK_LE(size_, kMaxSafeSize);
