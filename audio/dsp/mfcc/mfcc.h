...
class Mfcc {
 public:
  ...
  void set_upper_frequency_limit(double upper_frequency_limit) {
    if (initialized_) {
     std::cerr << "Set frequency limits before calling Initialize." << std::endl;
     exit(EXIT_FAILURE);
    }
  ...
  void set_filterbank_channel_count(int filterbank_channel_count) {
    if (initialized_) {
      std::cerr << "Set channel count before calling Initialize." << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  void set_dct_coefficient_count(int dct_coefficient_count) {
    if (initialized_) {
      std::cerr <<  "Set coefficient count before calling Initialize." << std::endl;
      exit(EXIT_FAILURE);
    }
