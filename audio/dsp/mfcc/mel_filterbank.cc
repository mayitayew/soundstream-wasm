...
bool MelFilterbank::Initialize(int fft_length, double sample_rate,
  ...
  if (num_mel_channels_ < 1) {
    std::cerr << "Number of mel filterbank channels must be positive.";
    return false;
  }

  if (sample_rate_ <= 0) {
    std::cerr << "Sample rate must be positive.";
    return false;
  }

  if (fft_length_ < 2) {
    std::cerr << " FFT length must greater than 1.";
    return false;
  }

  if (lower_frequency_limit < 0) {
    std::cerr << "Lower frequency limit must be nonnegative.";
    return false;
  }

  if (upper_frequency_limit <= lower_frequency_limit) {
    std::cerr << "Upper frequency limit must be greater than "
...
//      DCHECK_GT(channel, -2);
//      DCHECK_LT(channel, num_mel_channels_);
  ...
  if (!bad_channels.empty()) {
    std::cerr << "Missing " << bad_channels.size() << " bands "
...
void MelFilterbank::Compute(const std::vector<double> &squared_magnitude_fft,
  ...
  if (!initialized_) {
    std::cerr << "Mel Filterbank not initialized.";
    return;
  }

  if (squared_magnitude_fft.size() <= end_index_) {
    std::cerr << "FFT too short to compute filterbank";
    return;
  }
...
void MelFilterbank::EstimateInverse(
  ...
  if (mel.size() != num_mel_channels_) {
    std::cerr << "Mel size does not match number of mel channels.";
    return;
  }
