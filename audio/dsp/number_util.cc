...
ArithmeticSequence::ArithmeticSequence(double base, double step, double limit)
  ...
  if (!std::isfinite(base_) || !std::isfinite(limit_) || !std::isfinite(step_) ||
        step_ == 0.0) {
    std::cerr << "FATAL: " << "Arguments: (" << base << ", " << step << ", " << limit << ")";
    exit(EXIT_FAILURE);
  }
  ...
  if (sized > std::numeric_limits<int>::max()) {
    std::cerr << "FATAL: sized must be less than int_max. " << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  if (sign_step * limit_ + 2 * tol <
      sign_step * (base_ + step_ * (size_ - 1))) {
    std::cerr << "FATAL: "
              << std::setprecision(std::numeric_limits<double>::digits10 + 2)
              << "size = " << size_
              << " would overshoot limit in ArithmeticSequence(" << base_
              << ", " << step_ << ", " << limit_ << ")." << std::endl;
    exit(EXIT_FAILURE);
  }
  //  CHECK_LT(sign_step * limit_, sign_step * (base_ + step_ * size_))
  //      << std::setprecision(std::numeric_limits<double>::digits10 + 2)

  if (sign_step * limit_ >= sign_step * (base_ + step_ * size_)) {
    std::cerr << "FATAL: "
              << "size = " << size_
              << " would undershoot limit in ArithmeticSequence(" << base_
              << ", " << step_ << ", " << limit_ << ")." << std::endl;
    exit(EXIT_FAILURE);
  }
...
void CombinationsIterator::Next() {
  if (is_done_) {
    std::cerr << "ERROR: Next() called when already done." << std::endl;
    return;
  }
  int i;
  for (i = k_ - 1; i >= 0; --i) {
...
bool CrossProductRange::Iterator::operator==(
  ...
  return (is_end_ && other.is_end_) ||
         (is_end_ == other.is_end_ && shape_ == other.shape_ &&
...
inline Fraction AppendContinuedFractionTerm(const Fraction& convergent,
                                            const Fraction& prev_convergent,
                                            int term) {
  // DCHECK_GE(term, 1);
...
std::pair<int, int> RationalApproximation(double value, int max_denominator) {
  ...
  ::RationalApproximation(value, max_denominator, nullptr, &rational.first,
                          &rational.second);
  return rational;
}
