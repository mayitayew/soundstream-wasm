...
#include "audio/third_party/fft2d/fft.h"
...
std::vector<double> InverseSpectrogram::GenerateSynthesisWindow(
 ...
 // const int frame_length = analysis_window.size();
  ...
  for (int i = 0; i < step_length; ++i) {
    ...
    //DCHECK_NE(denom, 0.0);  // Can't reconstruct if the weight is zero.
    for (int offset = i; offset < analysis_window.size();
         offset += step_length) {
      result[offset] /= denom;
    }
...
bool InverseSpectrogram::Initialize(int fft_length, int step_length) {
  initialized_ = false;
  if (fft_length < 2) {
    std::cerr << "FFT length too short.";
    return false;
  }
  if (!IsPowerOfTwoOrZero(fft_length)) {
    std::cerr << "FFT length not a power of 2.";
    return false;
  }
...
bool InverseSpectrogram::Initialize(const std::vector<double>& window,
  ...
  initialized_ = false;
  if (window.size() < 2) {
    std::cerr << "Window length too short.";
    return false;
  }
...
bool InverseSpectrogram::Process(
  ...
  if (!initialized_) {
    std::cerr << "Process() called before successful call to Initialize().";
    return false;
  }
  //CHECK(output);
  output->clear();
...
void InverseSpectrogram::ProcessCoreFFT() {
  ...
  //DCHECK_EQ(overlap_, working_output_.size());
...
bool InverseSpectrogram::Flush(std::vector<OutputSample>* output) {
  ...
  if (!initialized_) {
    std::cerr << "Flush() called before successful call to Initialize().";
    return false;
  }
