...
private:
 ...
 void ProcessSamplesCommon(Input&& input, Output&& output) {
   ...
   if /*constexpr*/ (Input::Dims == 1) {
     //      CHECK_EQ(input.size() % num_channels_, 0)
   ...
   if /*constexpr*/ (Output::Dims == 1) {
     if (!output.resize(num_channels_ * num_output_frames)) {
       std::cerr  << "Expected output.size() == num_channels * num_output_frames == "
                 << num_channels_ << " * " << num_output_frames
                 << ", got: " << output.size()
                 << ". Use NextNumOutputFrames() to get the correct output size." << std::endl;
       exit(EXIT_FAILURE);
     }
     ...
     if (!output.resize(num_output_frames)) {
       std::cerr << "Expected output.size() == num_output_frames == "
     ...
     if (!output.resize(num_channels_, num_output_frames)) {
       std::cerr << "Expected output shape (num_channels, num_output_frames) == ("
...
struct UnpackTemplateArg {
 ...
 static void ProcessSamplesGeneric(
   ...
   int phase = phase_ref;
   //    DCHECK_EQ(input.rows(), num_channels);
   //    DCHECK_EQ(output.rows(), num_channels);
   //    DCHECK_LT(delayed_input_frames, num_taps);
   //    DCHECK_LT(phase, factor_denominator);
   ...
   while (i < i_straddle_end) {
     //DCHECK_LT(num_written, num_output_frames);
   ...
   if (i < delayed_input_frames) {
     ...
     // the input.
     //DCHECK_EQ(num_written, num_output_frames);
