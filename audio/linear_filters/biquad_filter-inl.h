...
void BiquadFilter<_SampleType>::Init(int num_channels,
  ...
  if (kNumChannelsAtCompileTime == Eigen::Dynamic) {
    ...
    if (num_channels != kNumChannelsAtCompileTime) {
      fprintf(stderr, "Number of channels must be %d.\n",
              kNumChannelsAtCompileTime);
      exit(EXIT_FAILURE);
    }
  }
  if (coeffs.a[0] == 0.0) {
    fprintf(stderr, "Filter coefficient a0 cannot be zero.\n");
    exit(EXIT_FAILURE);
  }
...
void BiquadFilter<_SampleType>::Reset() {
  if (num_channels_ < 1) {
        fprintf(stderr, "Number of channels must be at least 1. Reset called before Init().\n");
        exit(EXIT_FAILURE);
  }
...
void BiquadFilter<_SampleType>::ProcessSample(const InputType& input,
  ...
  using OutputScalarType =
      typename Traits::template GetScalarType<OutputType>::Type;
  constexpr int kFixedNumChannels =
      Traits::template GetFixedNumChannels<InputType, OutputType>();
//  DCHECK_GE(num_channels_, 1) << "ProcessSample() called before Init().";
...
//        << "Cannot operate on map with inner stride.";
    ...
    for (int channel = 0; channel < num_channels_; ++channel) {
      ...
      const AccumType next_state =
          input_data[channel] - (state_.row(channel) * feedback12_)[0];
    ...
    //DCHECK_EQ(Traits::AsEigenArray(input).rows(), 1);
    ...
    // Compute the output y[n] = b0 * s[n] + b1 * s[n - 1] + b2 * s[n - 2].
    output_data[channel] =
        next_state * feedforward0_ + (state_.row(channel) * feedforward12_)[0];
    // Update state by sliding.
    state_(channel, 1) = state_(channel, 0);
    state_(channel, 0) = next_state;
  } else {
...
//        << "Cannot operate on map with inner stride.";
    ...
    Eigen::Map<Eigen::Matrix<AccumType, kFixedNumChannels, 2>,
               ((sizeof(AccumType) * kFixedNumChannels * 2) % 16 == 0)
                   ? Eigen::Aligned
                   : Eigen::Unaligned>
        state_map(state_.data(), kFixedNumChannels, 2);
    ...
    Traits::AsMutableEigenArray(output)->matrix().noalias() =
        (next_state * feedforward0_ + state_map * feedforward12_)
            .template cast<OutputScalarType>();
    // Update state by sliding.
...
//  DCHECK_GE(num_channels, 1);
//  DCHECK_GE(all_coefficients.coeffs.size(), 1);
  num_channels_ = num_channels;
