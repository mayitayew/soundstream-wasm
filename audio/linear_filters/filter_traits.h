 ...
 public:
  ...
  struct ProcessBlockDynamicHelper {
    ...
    static void Run(FilterType* filter,
      ...
      if (input.innerStride() != 1) {
        fprintf(stderr, "Cannot operate on map with inner stride\n");
        exit(EXIT_FAILURE);
      }
      if (output->innerStride() != 1) {
        fprintf(stderr, "Cannot operate on map with inner stride\n");
        exit(EXIT_FAILURE);
      }
  ...
  static void ProcessBlock(FilterType* filter,
                           const InputType& input, OutputType* output) {
    CHECK_EQ(filter->num_channels(), input.rows());
    if (filter->num_channels() != input.rows()) {
      fprintf(stderr, "Number of channels in filter and input rows match\n");
      exit(EXIT_FAILURE);
    }
