...
std::unique_ptr<TfLiteModelWrapper> TfLiteModelWrapper::Create(
  ...
  if (model == nullptr) {
    std::cerr << "Could not build TFLite FlatBufferModel from buffer"
              << std::endl;
    return nullptr;
  }
  ...
  if (builder.SetNumThreads(1) != 0) {
    std::cerr << "Failed to SetNumThreads in TFLite interpreter.";
    return nullptr;
  }

  std::unique_ptr<tflite::Interpreter> interpreter;
  if (builder(&interpreter) != kTfLiteOk) {
    std::cerr << "Could not build TFLite Interpreter from model buffer."
              << std::endl;
    return nullptr;
  }
  ...
  if (use_xnn) {
    ...
    if (status == kTfLiteDelegateError) {
      std::cout << "Failed to set delegate; continuing without.";
    } else if (status != kTfLiteOk) {
      std::cerr << "Failed to set delegate, and cannot continue.";
      return nullptr;
    }
  }
  ...
  if (interpreter->AllocateTensors() != kTfLiteOk) {
    std::cerr << "Could not allocate quantize TFLite tensors for model buffer."
              << std::endl;
    return nullptr;
  }
...
std::unique_ptr<TfLiteModelWrapper> TfLiteModelWrapper::Create(
  ...
  std::unique_ptr<tflite::Interpreter> interpreter;
  if (builder(&interpreter) != kTfLiteOk) {
    std::cerr << "Could not build TFLite Interpreter for file: " << model_file;
    return nullptr;
  }
  ...
  if (interpreter->AllocateTensors() != kTfLiteOk) {
    std::cerr << "Could not allocate quantize TFLite tensors for file: "
              << model_file;
    return nullptr;
  }
